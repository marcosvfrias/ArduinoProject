...
def change_color(boton):
    style.configure('Custom.TButton', background='#50bbf0')
    boton.configure(style='Custom.TButton')

def input_color(input):
    style.configure('Custom.TEntry', background='#a4a4a4')
    input.configure(style='Custom.TEntry')

root = tk.Tk()
root.title("Monitoring")
root.iconphoto(True, PhotoImage(file="python-logo.png"))
...
root.bind("<Map>", lambda event: change_color(btn_mntr))
...
input1.bind("<Button-1>", lambda e: input1.delete(0, tk.END), input_color(input1))
...
input2.bind("<Button-1>", lambda e: input2.delete(0, tk.END), input_color(input2))
...
input3.bind("<Button-1>", lambda e: input3.delete(0, tk.END), input_color(input3))
...
out1.pack(padx=10, pady=10, ipady=5, ipadx=30)
...
out2.pack(padx=10, pady=10, ipady=5, ipadx=30)
...
out3.pack(padx=10, pady=10, ipady=5, ipadx=30)
...
out4.pack(padx=10, pady=10, ipady=5, ipadx=30)
